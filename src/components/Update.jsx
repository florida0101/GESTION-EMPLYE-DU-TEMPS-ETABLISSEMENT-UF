...
import SuccessDialogUpdate from "./successDialogUpdate";
import { fetchWeekDays } from "./requests";
import { updateExistingProgram } from "./requests";
...
function Update({ parcours, open, onClose }) {
    ...
    const handleUpdatingFieldsValue = () => {
    ...
    const handleSendingNewPrograms = async (event) => {
        ...
        const response = updateExistingProgram(schedules);
    ...
    return (
        <div>
        <SuccessDialogUpdate open={openSuccess} onClose={() => setOpenSuccess(false)} />
